...
<head>
    ...
    <style>
        ...
        h2 {
            color: #d9534f;
            /* Color retro para el título */
            font-weight: bold;
        }

        .text-primary {
            color: #5bc0de !important;
            /* Color retro para el texto resaltado */
        }

        .form-label {
            color: #5bc0de;
            /* Color retro para las etiquetas */
            font-weight: bold;
        }
...
<body>
    <div class="container mt-5">
        <h2 class="mb-4">Editar Criterio de Evaluación - <span
                class="text-primary"><?= htmlspecialchars($_GET['nombre_ra']) ?></span></h2>

        <form
            action="../Controllers/CriterioEvaluacionController.php?action=editar&id=<?= $criterio->getId_criterio() ?>&id_resultado_aprendizaje=<?= $_GET['id_resultado_aprendizaje'] ?>&nombre_ra=<?= urlencode($_GET['nombre_ra']) ?>"
            method="POST">
            ...
            <div class="mb-3">
                ...
                <input type="text" class="form-control" id="descripcion" name="descripcion"
                    value="<?= htmlspecialchars($criterio->getDescripcion()) ?>" required>
            </div>

            <div class="mb-3">
                <label for="orden" class="form-label">Código</label>
                <input type="number" class="form-control" id="orden" name="orden" value="<?= $criterio->getOrden() ?>"
                    min="1" required>
            </div>
